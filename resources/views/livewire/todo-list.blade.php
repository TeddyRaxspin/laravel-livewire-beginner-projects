<div class="flex flex-col w-[300px] mx-auto py-16">
    <div class="flex gap-4 justify-between">
        <input type="text" wire:model="todoText" wire:keydown.enter="addTodo"
            placeholder="Type and Hit Enter" class="flex-1" >
    ...
    <div class="py-6">
        ...
        @endif
            ...
            @foreach($todos as $index => $todo)
                <div class="flex gap-4 justify-between items-center py-1 text-gray-500">
                    <input type="checkbox" {{$todo->completed ? 'checked' : ''}} wire:change="toogleTodo({{$todo->id}})">
                    <label class="flex-1 {{$todo->completed ? 'line-through text-red-700' : ''}}">{{$todo->todo}}</label>
