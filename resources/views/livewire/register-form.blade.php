    ...
    <form wire:submit.prevent="submit" class="w-[400px] mx-auto py-16">
        ...
        @endif
            ...
            <div class="flex gap-4 mb-4 text-gray-500">
                <label>
                    <input type="radio" value="customer" name="role" wire:model="role" class="mr-1">Customer
                </label>
                <label class="px-2">
                    <input type="radio" value="vendor" name="role" wire:model="role" class="mr-1">Vendor
                </label>
            </div>

            <div class="mb-4">
                <input type="text" wire:model.debounce.500ms="first_name"
                       class="w-full border @error('first_name') border-red-600 @enderror"
                       placeholder="First Name">
                @error('first_name')<span class="text-red-600">{{$message}}</span> @enderror
            </div>
            <div class="mb-4">
                <input type="text" wire:model.debounce.500ms="last_name"
                       class="w-full border @error('last_name') border-red-600 @enderror"
                       placeholder="Last Name">
                @error('last_name')<span class="text-red-600">{{$message}}</span> @enderror
            </div>
            <div class="mb-4">
                <input type="email" wire:model.debounce.500ms="email"
                       class="w-full border @error('email') border-red-600 @enderror"
                       placeholder="Email">
                @error('email')<span class="text-red-600">{{$message}}</span> @enderror
            </div>
            <div class="mb-4">
                <input type="password" wire:model.debounce.500ms="password"
                       class="w-full border @error('password') border-red-600 @enderror"
            ...
            @if($role == 'vendor')
                <div class="mb-4">
                    ...
                    class="w-full border @error('company_name') border-red-600 @enderror"
                ...
                <div class="mb-4">
                    <input type="text" wire:model.debounce.500ms="vat_number"
                           class="w-full border @error('vat_number') border-red-600 @enderror"
